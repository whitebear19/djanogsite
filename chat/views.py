    ...
    try:
        ...
        for room in roomtemp:
            ...
            if rows.count() > 0:
                ...
                data['created_at'] = get_different_time(first_message.created_at)
...
def get_stored_message_nav(request): 
    ...
    try:
        roomtemp = Room.objects.filter(who=user.id)|Room.objects.filter(whom=user.id).order_by('-created_at')
        for room in roomtemp:
            rows = Message.objects.filter(room_id=room.id).exclude(user_id=user.id)
            if len(messages) < 4:
                if rows.count() > 0:
                    data = {}
                    count_message = rows.count()
...
def clear_message_alert(request):
    user = request.user    
    try:
        roomtemp = Room.objects.filter(who=user.id)|Room.objects.filter(whom=user.id).order_by('-created_at')
        for room in roomtemp:
